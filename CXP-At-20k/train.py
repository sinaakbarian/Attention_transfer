...
def ModelTrain(train_df_path, val_df_path, path_image, ModelType, CriterionType, device,LR):
    #Tensorboard
    writer = SummaryWriter("/h/sinaakb/Laleh-sina/CXP-sina/runs/reintiallized_AT-Mimic-alldata-7---")
    # Training parameters
    batch_size = 24
    ...
    N_LABELS = 14
    start_epoch = 0
    num_epochs = 128  # number of epochs to train for (if early stopping is not triggered)
    ...
    if ModelType == 'densenet':
        ...
        #number_labels_NIH=15
        #num_ftrs = model.classifier[0].in_features
        #model.classifier = nn.Sequential(nn.Linear(num_ftrs, number_labels_NIH),nn.Sigmoid())
